...
void atender_memoria_cpu(){
	...
	t_paquete* un_paquete = crear_paquete(PETICION_INFO_CPU_MEMORIA);
	cargar_int_al_paquete(un_paquete, TAM_PAGINA);
...
void escribir_valor_en_dir_fisica(t_buffer* un_buffer){
	...
	size_t tamanio = recibir_size_t_del_buffer(un_buffer);
	tipo_dato_parametro tipo_dato = recibir_int_del_buffer(un_buffer);
	void* valor = recibir_generico_del_buffer(un_buffer);
...
void enviar_instruccion(char* instruccion){
	// M -> CPU : [char* ]
	retardo_respuesta();
	t_paquete* un_paquete = crear_paquete(PETICION_DE_INSTRUCCIONES_CPU_MEMORIA);
...
void enviar_respuesta_por_consulta_de_pagina(int respuesta_a_cpu){
	...
	retardo_respuesta();
	t_paquete* un_paquete = crear_paquete(CONSULTA_DE_PAGINA_CPU_MEMORIA);
	cargar_int_al_paquete(un_paquete, respuesta_a_cpu);
	enviar_paquete(un_paquete, fd_cpu);
	eliminar_paquete(un_paquete);
}
...
void enviar_uint32_por_lectura(uint32_t valor, tipo_dato_parametro tipo_dato){
    // M -> CPU : [void* valor]
    retardo_respuesta();
	t_paquete* un_paquete = crear_paquete(LECTURA_BLOQUE_CPU_MEMORIA);
	cargar_int_al_paquete(un_paquete, tipo_dato);
...
void enviar_uint8_por_lectura(uint8_t valor, tipo_dato_parametro tipo_dato){
	...
	t_paquete* un_paquete = crear_paquete(LECTURA_BLOQUE_CPU_MEMORIA);
	cargar_int_al_paquete(un_paquete, tipo_dato);
    cargar_uint8_al_paquete(un_paquete, valor);
    enviar_paquete(un_paquete, fd_cpu);
    eliminar_paquete(un_paquete);
}
...
void enviar_string_por_lectura(char* valor, tipo_dato_parametro tipo_dato){
	...
	t_paquete* un_paquete = crear_paquete(LECTURA_BLOQUE_CPU_MEMORIA);
	cargar_int_al_paquete(un_paquete, tipo_dato);
    cargar_string_al_paquete(un_paquete, valor);
    enviar_paquete(un_paquete, fd_cpu);
    eliminar_paquete(un_paquete);
...
void enviar_valor_por_lectura(void* valor, tipo_dato_parametro tipo_dato, size_t tamanio){
	...
	t_paquete* un_paquete = crear_paquete(LECTURA_BLOQUE_CPU_MEMORIA);
	cargar_int_al_paquete(un_paquete, tipo_dato);
	cargar_generico_al_paquete(un_paquete, valor, tamanio);
    enviar_paquete(un_paquete, fd_cpu);
    eliminar_paquete(un_paquete);
...
void enviar_respuesta_por_escritura(){
	//M -> CPU : [char* OK]
	retardo_respuesta();
	t_paquete* un_paquete = crear_paquete(ESCRITURA_BLOQUE_CPU_MEMORIA);
	cargar_string_al_paquete(un_paquete, "SE ESCRIBIO EN LA MEMORIA");
	enviar_paquete(un_paquete, fd_cpu);
	eliminar_paquete(un_paquete);
}
...
void enviar_respuesta_por_resize(){
	//M -> CPU : [char* OK]
	retardo_respuesta();
	t_paquete* un_paquete = crear_paquete(RESIZE_CPU_MEMORIA);
...
void enviar_OUT_OF_MEMORY(){
	retardo_respuesta();
	t_paquete* un_paquete = crear_paquete(RESIZE_CPU_MEMORIA);
	cargar_string_al_paquete(un_paquete, "OUT OF MEMORY");
	enviar_paquete(un_paquete, fd_cpu);
	eliminar_paquete(un_paquete);
}
