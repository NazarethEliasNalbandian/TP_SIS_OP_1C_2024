...
void _gestionar_interrupt_consola(){
	while(1){
		...
		cargar_int_al_paquete(un_paquete, pcb_execute->pid);
		cargar_int_al_paquete(un_paquete, pcb_execute->verificador);

		if(pcb_execute->flag_finalizar_proceso){
			cargar_string_al_paquete(un_paquete, "DESALOJO_POR_CONSOLA");
...
void _gestionar_interrupt_quantum(){
	while(1){
		...
		if(pcb_execute->flag_finalizar_proceso){
			...
			cargar_string_al_paquete(un_paquete, "FIN_QUANTUM");
			enviar_paquete(un_paquete, fd_cpu_interrupt);
			eliminar_paquete(un_paquete);
		}
	}
...
void _gestionar_interrupt_prioridad(){
	while(1){
		...
		if(pcb_execute->flag_finalizar_proceso){
			...
			cargar_string_al_paquete(un_paquete, "PRIORIDAD");
			enviar_paquete(un_paquete, fd_cpu_interrupt);
			eliminar_paquete(un_paquete);
